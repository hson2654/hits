#### Info Emulate
This machine is odd, ICMP is blocked, seems only port 80 is open.

For port 80, a file upload function. only odt extention is allowed

https://www.exploit-db.com/exploits/44564

get a hash of user, but seems not crackable.
```
...
End Sub
```
upload it and get a shell of a user.

get info from winpeas
```
...
```
`net user`
found apache is one of user. so I search for win server folder.
...
```
It is the root folder of the port 80 service.

`icacls .`
to identify the priv on this path, C:\xampp\htdocs. I have write and read priv. So write a Cmd page in it, then I can access it from kali via port 80.
...
craft\apache
```
It works, then get reverse shell using nc
...
#### Lesson learned
- create odt/dopc Macros shell payload
- icacls to find priv of folder
